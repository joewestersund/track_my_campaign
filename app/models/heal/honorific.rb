...
class Heal::Honorific < ActiveRecord::Base
  ...
  def create_update_description
    if created_at == updated_at
      {type: :honorific, description: "Honorific #{name} was created", date: created_at, obj:self}
    else
      {type: :honorific, description: "Honorific #{name} was updated", date: updated_at, obj:self}
    end
  end
end
